...
The importance of ecoinformatics is housed in its ability to "allow a broader perspective over time and space, and across many disciplines, than is possible from one or a few studies," and in allowing data to be reused for questions that may emerge in the future (thus may fit well with "biocomplex" framework).
...
The response of ecoinformatics to resolving issues related to data islands are largely:
  ...
  - *Data collections (metadata-driven)* - collection of datasets that are loosely structured together by well-crafted metadata. A good example if the Global Biodiversity Information Facility (GBIF), which compiles data from 450 museum collections. Also, Data Dryad, which links datasets to papers and is a member of the dataOne network. Some advantages of meta-data driven databases are:
...
**Key contribution:** This is the lead article to a special issue in *Journal of Vegetation Science* that focuses on ecoinformatics driven approaches to understanding complexity within vegetation systems. 

The article is less informative in its own right as much as a good pointer to other key studies working with vegetation science and ecoinformatics. It appears to be one of the earliest explicit applications of ecoinformatics to vegetation science.
...
Biocomplexity appears to be thinking about ecology through a hierarchical lens, in which processes that exist at more baseline levels (physics, chemistry, etc.) result in ecological processes > functions > services.
...
*Emergent properties* - concept that the whole is greater than the sum of its parts when examining ecological phenomena and the underlying processes that drive them. Emergent properties can exist at any scale, and may build upon themselves 
  ...
  - e.g., morphology and physico-chemical conditions leading to ecophysiology (emergent property) which then leads to spatial assemblages (2^nd^ emergent property), etc.). 
...
**Key notes: ** The study notes several key areas within the data life cycle:
  ...
  3. Data integration (2): Integrating data from local observations to models and back requires unique considerations
...
**Key significance:** This study calls for more explicit investigation of cross scale interactions (CSIs) both in time and space. They lay out a framework for investigating CSIs that consists of: 

  1. Conceptual mapping 
  2. Construction of an appropriate database (ecoinformatics)
  3. Modeling strategies that specifically investigate interactions (e.g., interaction terms in hierarchical modeling).
...
*Steps for quantifying CSIs*:
  ...
  3. Test for CSI as interaction term using hierarchical models
...
The "catastrophic shift" is the point at which the bifurcation point is passed and the system shifts from a stable state to an unstable state or alternatively to an alternative stable state.
...
Desertification - Presence of desert vs vegetated landscapes may be conceived of as alternative stable states. May occur through time as vegetation promotes more moist conditions and lack of vegetation promotes drier conditions. May have feedbacks with the climate in the Sahel region that produce alternating stable states.
...
One other that they do not discuss but has shown up in other literature and may be relevant for intertidal ecosystems is:
  ...
  1. Self-organized patchiness (see Section \@ref(eslami2015)).
...
### Johnson *et al*, 2017 {#johnson2017}

*"Knowing when (not) to attempt ecological restoration"* [@johnson2017knowing]

**Key significance: ** This opinion piece examines the critical questions of:

  1. How do we identify when community change represents a phase shift to a different ecological dynamic that warrants considering restoration?
  2. How do we identify discontinuous phase shift and hysteresis that may render futile attempts at restoration?
...
The authors lay out two key considerations that must be undertaken prior to restoration:
  ...
  1. Must interpret ecological change; typical fluctuations represent a properly functioning ecosystem that does not warrant restoration, whereas other less typical fluctuations may indicate loss of resilience or regime shift
  2. Ecological change may involve hysteresis and thus consideration as to whether or not restoration will be effective must be taken
...
### Eslami-Andergoli, 2015 {#eslami2015}
...
### Ellison, 2015 {#ellison2015}

*"Vulnerability assessment of mangroves to climate change and sea-level rise impacts"* [@ellison2015vulnerability]
...
The vulnerability assessment methodology seeks to identify aspects that may already be experience climate change impacts and may also be more susceptible to future impacts.
...
*"Can mangroves keep pace with sea level rise? A global data review"* [@sasmito2016can]

**Key contribution: ** This is a well-written study that looks at data of relative sea level rise and accretion rates in mangroves and assesses the abundance of empirical evidence indicating whether or not mangroves may keep pace with sea level rise. The article is summarized in section \@ref(sasmito2016).
